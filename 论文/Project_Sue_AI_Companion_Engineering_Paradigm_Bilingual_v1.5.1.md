...
**Zenodo DOI (v1.5):** https://doi.org/10.5281/zenodo.22022948  
**Version:** 1.5.1 Public Paradigm Draft  
**Date:** 2026-08-21
...
**为什么是“状态”而不是“数字”？** Project Sue 明确拒绝把伴侣建模成 galgame 式的数值系统（好感度、体力、兴奋值等精确数字）。这不是因为数值无法实现，而是因为 LLM 并不天然地“需要”数字——它需要的是能够触发语义扩展的**状态锚点（state anchor）**。所有底层数字——疲劳、兴奋、温度、时长——都先由数字身体翻译成可解释的状态（如 fresh、warm、wanting），再注入给模型；模型只接收“状态”，不接触“数字”。动机在于：数字对模型而言是噪音，状态才是语义；把数值翻译成状态，正是“程序保证事实、人格解释事实”这一原则在身体维度上的体现。
...
**Why “state” rather than “numbers”?** Project Sue explicitly rejects modeling the companion as a galgame-style numeric system (affection points, stamina bars, arousal values, etc.). This is not because numbers cannot be implemented, but because an LLM does not inherently “need” numbers—it needs a **state anchor** that can trigger semantic expansion. All underlying numeric values—fatigue, arousal, temperature, duration—are first translated by the digital body into interpretable states (e.g., fresh, warm, wanting) and only then injected into the model; the model receives “state” and never touches “numbers”. The motivation is: numbers are noise to a model, while state is semantics. Translating values into states is the same principle—programmatic facts, persona-level meaning—applied to the bodily dimension.
...
- **v1.5.1（2026-08-21）**：小修改——第 7 章“数字身体”补充“为什么是状态而不是数字”段落（拒绝 galgame 数值系统，数字翻译为状态锚点，LLM 只接收状态、不接触数字），中英双语。
...
- **v1.5.1 (2026-08-21):** Minor revision—added the “Why state rather than numbers” paragraph to Section 7 (Digital Body): rejecting galgame-style numeric systems, translating numbers into state anchors, and having the LLM receive only state, never numbers; bilingual.
