...
静雯在 23:30 cron 晚安后已进入休息。23:40 起，心跳每轮检查最近一个"应该存在但不存在"的 shadow daily：
- 从今天往前找——哪一天有雯时段互动标记但 shadow 文件缺失？
- 找到了 → 雯自主切换入场，补写那一天的 shadow daily
- 全都有 → 静默跳过
- 无回复写入——老刘不看到切换过程，只看到 shadow 已补的结果
